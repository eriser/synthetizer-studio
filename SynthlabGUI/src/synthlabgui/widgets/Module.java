...
public class Module extends JPanel implements MouseListener,
	...
	configWindow_ = new ModuleConfigWindow(module_, null, new Point(0, 0));
    }
	...
	// Module name
	g.drawString(module_.getName(), 100 - (g.getFontMetrics().stringWidth(
	...
	if (x < 0)
	    x = 0;
	else if (x + getWidth() >= poolPanel.getWidth())
	    x = poolPanel.getWidth() - getWidth();
	if (y < 0)
	    y = 0;
	else if (y + getHeight() >= poolPanel.getHeight())
	...
	if (e.getClickCount() == 2 && module_.getInputs().size() > 0) {
		...
		configWindow_ = new ModuleConfigWindow(module_,
