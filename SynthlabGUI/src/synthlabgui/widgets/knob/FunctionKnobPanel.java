...
public class FunctionKnobPanel extends JPanel implements KnobListener, AbstractConfigPanel
{
  ...
  public void paintComponent(Graphics gc)
  {
    ...
    gc.drawImage(icon2.getImage(), getWidth() - imgSize - 2, TITLE_HEIGHT + imgSize + (AbstractKnob.size.height / 2)
        - 5, null);
  ...
  private Dimension         dim              = new Dimension((imgSize * 2) + AbstractKnob.size.width + 2,
                                                 NumberKnobPanel.TITLE_HEIGHT + (imgSize * 2)
                                                     + AbstractKnob.size.height + 2);
}
