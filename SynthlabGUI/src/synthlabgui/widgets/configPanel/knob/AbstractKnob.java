...
public abstract class AbstractKnob extends JPanel implements MouseListener, MouseMotionListener
{
  ...
  public void paint(Graphics gc)
  {
     ...
     * RenderingHints.VALUE_RENDER_QUALITY); ((Graphics2D) gc).setRenderingHints(renderHints); gc.setColor(Color.white);
     * gc.fillOval(0, 0, getWidth(), getHeight()); gc.setColor(Color.black); gc.drawOval(0, 0, getWidth() - 1,
     * getHeight() - 1); gc.setColor(Color.red); gc.drawLine(centerPoint.x, centerPoint.y, pointer.x, pointer.y);
     */
    ...
    g2.fillArc(2, 2, 47, 47, (int) (((5. * Math.PI) / 4.) / (2. * Math.PI) * 360.), -(int) ((value / 10000.)
        * ((6. * Math.PI) / 4.) / (2. * Math.PI) * 360.));
    // ruler outer
    g2.setColor(new Color(240, 240, 240));
    g2.drawArc(2, 2, 47, 47, (int) (((7. * Math.PI) / 4.) / (2. * Math.PI) * 360.), (int) (((6. * Math.PI) / 4.)
        / (2. * Math.PI) * 360.));
    // ruler lines
    for (double i = -2; i <= 10; ++i)
      g2.drawLine(25, 25, 25 + (int) (Math.cos(i * Math.PI / 8.) * 24.), 25 - (int) (Math.sin(i * Math.PI / 8.) * 24.));
    // Knob inner
    GradientPaint innerGradient = new GradientPaint(new Point(0, 0), new Color(220, 220, 220), new Point(50, 50),
        new Color(180, 180, 180));
    g2.setPaint(innerGradient);
    g2.fillOval(5, 5, 40, 40);
    // Knob outer
    GradientPaint outerGradient = new GradientPaint(new Point(0, 0), new Color(120, 120, 120), new Point(50, 50),
        new Color(80, 80, 80));
    g2.setPaint(outerGradient);
    g2.drawOval(5, 5, 40, 40);
    // rotation
    g2.rotate((value / 10000. * 6. * (Math.PI / 4.)) + 5. * (Math.PI / 4.), 25, 25);
    // selector inner
    GradientPaint innerSelectorGradient = new GradientPaint(new Point(0, 0), new Color(150, 150, 150),
        new Point(20, 0), new Color(250, 250, 250));
    g2.setPaint(innerSelectorGradient);
    g2.fillOval(15, 5, 20, 10);
    // selector outer
    GradientPaint outerSelectorGradient = new GradientPaint(new Point(0, 0), new Color(160, 160, 160),
        new Point(50, 50), new Color(120, 120, 120));
