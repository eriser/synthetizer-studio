...
public class ModuleConfigWindow extends JDialog {
     ...
     * */
    public ModuleConfigWindow(String name, synthlab.api.Module module,
	    MainWindow parent, Point point) {
	super();
	this.name = name;
	setModal(false);
	setAlwaysOnTop(true);
	...
	if (port.getValueType() == Port.ValueType.WAVE_SHAPE) {
	    ...
	    double min = port.getValueRange().minimum;
	    double max = port.getValueRange().maximum;
	    panel = new NumberKnobPanel(port.getName(), min, max, unitList
	...
	} else if (port.getValueType() == Port.ValueType.SWITCH) {
