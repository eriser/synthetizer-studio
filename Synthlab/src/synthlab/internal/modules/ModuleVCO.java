...
public class ModuleVCO extends BasicModule
{
  ...
  public void compute()
  {
    ...
    {
      out = (Math.sin(((double) frameCount_ / (double) frameRate_) * (Math.pow( 2, ifreq + iconst )*initialFrequency_) * 2. * Math.PI)) >= 0 ? 1. : -1;
    }
    else if ( ishape <= SHAPE_SINE )
    {
      out = Math.sin(((double) frameCount_ / (double) frameRate_) * (Math.pow( 2, ifreq + iconst )*initialFrequency_) * 2. * Math.PI);
    }
